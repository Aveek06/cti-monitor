...
async function dispatch(req, res, segments) {
  ...
  try {
    if (path === '' || path === 'discovery') return await handleDiscovery(res);
    if (path === 'collections') return await handleCollections(res);
    const colMatch = path.match(/^collections\/([^/]+)$/);
    if (colMatch) return await handleCollectionInfo(colMatch[1], res);
    const objMatch = path.match(/^collections\/([^/]+)\/objects$/);
    if (objMatch) {
    ...
    return res.status(404).json({ title: 'Not Found', http_status: 404, description: `Unknown TAXII path: ${path}` });
  } catch (err) {
...
module.exports = async function handler(req, res) {
  ...
  return dispatch(req, res, req.query.segments || []);
};
